    ...
    function ($scope, $rootScope, $http, $location, $window, $filter, Notification) {
        ...
        $scope.next_wizardEdit = function (acId, grdID) {
            if (acId == null || grdID == null || acId == "" || grdID == "") {
                Notification.error({
                    message: 'Select Level and Academic Year',
                    positionX: 'center',
                    delay: 2000
                });
            } else {
        ...
        $scope.inactiveButton = function (){
            ...
            $scope.getPaginatedStudentList();

        };

        $scope.getPaginatedStudentList = function (page) {
            switch (page) {
                case 'firstPage':
            ...
            var studentText=$scope.studentText;
            if(angular.isUndefined(grade)||grade==null){
                grade="";
                $scope.gradeNameText=null;
            }
            if(angular.isUndefined(studentText)||studentText==null){
                studentText="";
                $scope.studentText=null;
            }
            $http.post($scope.bshimServerURL  + '/getPaginatedStudentList?searchText=' + $scope.SearchText+'&checkboxStatusForStudent='+ $scope.inactiveStatus +'&gradeSearch='+grade+'&studentSearch='+studentText,angular.toJson(paginationDetails)).then(function (response) {
        ...
        $scope.saveStudentImport = function(){
            $scope.isDisabled= true;
            var formElement = document.getElementById("studentDetails");
            var studentDetails = new FormData(formElement);
            $http.post($scope.bshimServerURL + '/studentImportSave',studentDetails,
                { headers: {'Content-Type': undefined},
                    transformRequest: angular.identity,
                }).then(function (response) {
                    $("#import_student").modal('hide');
                    $scope.getPaginatedStudentList();
        ...
        $scope.saveFeeImport = function(){
            $scope.isDisabled= true;
            var formElement = document.getElementById("studentFeeDetails");
            var studentDetails = new FormData(formElement);
            $http.post($scope.bshimServerURL + '/feeImportSave',studentDetails,
                { headers: {'Content-Type': undefined},
                    transformRequest: angular.identity,
                }).then(function (response) {
                    $("#import_student").modal('hide');
        ...
        $scope.studentFeeDetailsList = function (student) {
            ...
            $http.post($scope.bshimServerURL + "/getStudentDetails?studentId=" + student.studentId+'&type='+"Student").then(function (response) {
                ...
                student.acdYearId = data.academicYearId;
        ...
        $scope.saveStudent = function () {
            ...
            if (angular.isUndefined($scope.acdYearId) || $scope.acdYearId == '') {
                ...
                Notification.warning({message: 'Level Name cannot be Empty', positionX: 'center', delay: 2000});
                $scope.isDisabled= false;
            }
            ...
            else if (angular.isUndefined($scope.valueType) || $scope.valueType == '') {
                Notification.warning({message: 'Student Type cannot be Empty', positionX: 'center', delay: 2000});
                $scope.isDisabled= false;
            }
                    ...
                    if (dateofadmission > dateofjoining) {
                        ...
                        $http.post($scope.bshimServerURL + "/saveNewStudent", angular.toJson(saveStudentDetails)).then(function (response) {
                            ...
                            if (data == "") {
                                ...
                                data.primaryContactNo = $scope.contactNo;
                                $scope.sendSMS(data);
        ...
        $scope.editStudent = function (data, feeDetailsList) {
            $("#back").hide();
            $scope.setupobj = data;
            $scope.studentProfileId = data.studentProfileId;
            ...
            $scope.gradeId = parseInt(data.gradeId);
            ...
            $scope.valueType =data.studentType;
        ...
        $scope.sendSMS = function(student){
            ...
            if(student.type=="NewStudent") {
                $scope.numberList.push(student.primaryContactNo);
                $http.post($scope.bshimServerURL + '/sendSMS?type=' + "Admission", angular.toJson($scope.numberList)).then(function (response) {
                    var data = response.data;
                    $scope.smsData = data;
                })
            }
