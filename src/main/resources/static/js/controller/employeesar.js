app.controller('employeesarCntrl',
    function ($scope, $timeout, $http, $location, $filter, Notification) {
      ...
      $scope.next_wizardEdit6 = function () {
        ...
        $scope.getSarList = function (val) {
            if (angular.isUndefined(val)) {
                val = "";
            }

            $(".loader").css("display", "block");
            $http.post($scope.bshimServerURL  + '/getSarApplicList?searchText=' + val).then(function (response) {
        ...
        $scope.editEmployesar = function (data) {
            $scope.apprId = data.apprId,
        ...
        $scope.getEmployeeByName=function(type) {
        ...
        $scope.saveEmployeesar = function () {
            ...
            // else {
                ...
                $http.post($scope.bshimServerURL + '/saveSarDetails', angular.toJson(saveSarDetails, "Create")).then(function (response, status, headers, config) {
                    var data = response.data;
                    if (data == "") {
        ...
        $scope.removeSarDetails = function () {
            $scope.apprId = "";
            $scope.staffName = "";
            $scope.staffDesig = "";
            $scope.staffDepar = "";
