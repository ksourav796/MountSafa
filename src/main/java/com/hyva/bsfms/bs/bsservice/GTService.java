...
import com.hyva.bsfms.bs.bsmapper.BsUserMapper;
import com.hyva.bsfms.bs.bsmapper.GTMapper;
...
public class GTService {
    ...
    public  FormSetUp getFooter(String formSetupType) {
        ...
        try {
            formSetUp = posFormSetupRepository.findAllByTypename(formSetupType);
        ...
        try{
            ...
            if (opgtDTO.getBankPayment().getMultiBankPaymentList().size() != 0) {
                for (MultiBankPayment dto : opgtDTO.getBankPayment().getMultiBankPaymentList()) {
                    ...
                    if (org.springframework.util.StringUtils.pathEquals(dto.getPaymentType(), "Cheque")) {
...
//            posReceiptPaymentTypes.setContact(hiposDAO.getOtherContactsObject(Long.parseLong(otherReceipt.getOtherContactID())));
    ...
    public OtherRecieptGtDTO getPrintDetailsOtherRecipt(OtherRecieptGtDTO orgtDTO,OtherReceipt otherReceipt,List<OcOtherReceiptWithoutItem> sidList)throws ParseException{
        ...
        orgtDTO.setSchoolBranchDetails(schoolBranchDetails);
...
//                if (!bankPayment.getDate().after(new Date())) {
                ...
                createGeneralLedgerForGTPurchase(otherReceipt.getOrDate(),formSetUp, otherReceipt.getOrno(),bankPaymentACC, "bankPayment", bankVal,"",orgtDTO.getCutomerName(),orgtDTO.getCurrencyId(),orgtDTO.getExchangerateValue());
...
//            pid.setTaxId(tax);
    ...
    public List<PosInvoiceDTO> getOtherReceiptDuplicateList() {
        List<String> statusList=new ArrayList<>();
        statusList.add("Cancelled Invoice");
        statusList.add("Draft Cancelled Invoice");
      List<OtherReceipt> otherReceipt=otherReceiptRepository.findAllByStatusNotIn(statusList);
    ...
    public OtherRecieptGtDTO duplicateReceiptPrint(String paymentId){
        ...
        List<SchoolBranchDetails> company=schoolBranchDetailsRepository.findAll();
        SchoolBranchDetails schoolBranchDetails=schoolBranchDetailsRepository.findOne(company.get(0).getSchoolBranchId());
        opgtDTO.setSchoolBranchDetails(schoolBranchDetails);
    ...
    @Transactional
    public OtherPurchaseGtDTO createOtherPurchaseNPayment(@RequestBody OtherPurchaseGtDTO opgtDTO, int userId) throws IOException, JSONException, ParseException {
...
//                if (!bankPayment.getDate().after(new Date())) {
                AccountMaster bankPaymentACC = accountMasterRepository.findOne(bankPayment.getBankAccountId());
                BigDecimal bankVal = new BigDecimal(bankPayment.getAmount());
                createGeneralLedgerForGTPurchase(otherPayment.getOpDate(),formSetUp, otherPayment.getOpNo(),bankPaymentACC, "bankPayment", bankVal.negate(),opgtDTO.getSupplierInvNo(),opgtDTO.getSupplierName(),opgtDTO.getCurrencyId(),opgtDTO.getExchangerateValue());
...
//            accountGroupService.upDateTotalBalanceAccountGroup(map);
    ...
    public JournalEntryDTO getPrintDetailsJournalEntry(JournalEntryDTO jegtDTO,JournalEntry journalEntry, List<JournalEntryDetails> jeList){
        ...
        List<SchoolBranchDetails> company=schoolBranchDetailsRepository.findAll();
        SchoolBranchDetails schoolBranchDetails=schoolBranchDetailsRepository.findOne(company.get(0).getSchoolBranchId());
        jegtDTO.setSchoolBranchDetails(schoolBranchDetails);
        return jegtDTO;
    }
...
//        hiposLedgerDAO.createGTofRespectiveTable(dt,Tran_type, Tran_no,account, memo,amount, PLRNo,flag,fiscalYear,this.userAccountSetup,currency,currencyValue);
    ...
    public  List<JournalEntry> getOtherJournalEntryList(String itemSearchText) {
        List<JournalEntry> otherPaymentList = journalEntryRepository.findAll();
    ...
    public JournalEntryDTO duplicateJournalEntryPrint(String paymentId){
        ...
        List<SchoolBranchDetails> company=schoolBranchDetailsRepository.findAll();
        SchoolBranchDetails schoolBranchDetails=schoolBranchDetailsRepository.findOne(company.get(0).getSchoolBranchId());
        jeDTO.setSchoolBranchDetails(schoolBranchDetails);
        return jeDTO;
    }
    public List<OtherPayment> getOtherPaymentDuplicateList(String itemSearchText) {
        List<String> statusList=new ArrayList<>();
        statusList.add("Cancelled Invoice");
        statusList.add("Draft Cancelled Invoice");
        List<OtherPayment> otherPaymentList = otherPaymentRepository.findAllByStatusNotIn(statusList);
        return otherPaymentList;
    }
